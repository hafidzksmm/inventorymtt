...
export default {
  ...
  methods: {
    async fetchItems() {
      const res = await axios.get("http://192.168.20.17:50/api/assetjual/assetjual");
      this.items = res.data;
      this.generateFilterOptions();
    },

    async fetchLokasi() {
      const res = await axios.get("http://192.168.20.17:50/api/assetjual/lokasi");
      this.lokasiOptions = [
        { value: "ALL", text: "📍 Semua Lokasi" },
        { value: "", text: "Pilih Lokasi" },
    ...
    async createItem() {
      await axios.post("http://192.168.20.17:50/api/assetjual", this.form);
    ...
    async updateItem() {
      await axios.put(`http://192.168.20.17:50/api/assetjual/${this.form.id}`, this.form);
    ...
    async deleteItem(id) {
      if (confirm("Yakin ingin menghapus data ini?")) {
        await axios.delete(`http://192.168.20.17:50/api/assetjual/${id}`);
        this.fetchItems();
      }
    },
    ...
    async handleImportExcel() {
      ...
      reader.onload = async (e) => {
        ...
        await axios.post("http://192.168.20.17:50/api/assetjual/assetjual/import", withLoc);
