...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const res = await axios.get(
          "http://192.168.20.17:50/api/inventaryprojek/inventaryprojek"
        );
    ...
    async fetchLokasi() {
      try {
        const res = await axios.get(
          "http://192.168.20.17:50/api/inventaryprojek/lokasi"
        );
        this.lokasiOptions = [
          { value: "ALL", text: "📍 Semua Lokasi" },
          { value: "", text: "Pilih Lokasi" },
    ...
    async handleImportExcel() {
      ...
      reader.onload = async (e) => {
        ...
        try {
          await axios.post(
            "http://192.168.20.17:50/api/inventaryprojek/inventaryprojek/import",
            dataWithLocation
          );
    ...
    async saveItem() {
      try {
        if (this.form.id) {
          await axios.put(
            `http://192.168.20.17:50/api/inventaryprojek/inventaryprojek/${this.form.id}`,
            this.form
          );
        } else {
          await axios.post(
    ...
    async deleteItem(id) {
      if (confirm("Yakin hapus data ini?")) {
        try {
          await axios.delete(
            `http://192.168.20.17:50/api/inventaryprojek/inventaryprojek/${id}`
          );
          this.fetchData();
        } catch (err) {
          console.error(err);
