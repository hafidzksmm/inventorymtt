...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const res = await axios.get("http://192.168.20.17:5000/api/inventaris/inventaris");
        this.items = res.data;
    ...
    async handleImportExcel() {
      ...
      reader.onload = async (e) => {
        ...
        try {
          await axios.post(
            "http://192.168.20.17:5000/api/inventaris/inventaris/import",
            dataWithLocation
          );
    ...
    async createItem() {
      await axios.post("http://192.168.20.17:5000/api/inventaris/inventaris", this.form);
      this.isModalOpen = false;
      this.fetchData();
    },
    async updateItem() {
      await axios.put(
        `http://192.168.20.17:5000/api/inventaris/inventaris/${this.form.id}`,
        this.form
      );
      this.isModalOpen = false;
      this.fetchData();
    ...
    async deleteItem(id) {
      if (confirm("Yakin ingin menghapus data ini?")) {
        await axios.delete(`http://192.168.20.17:5000/api/inventaris/inventaris/${id}`);
        this.fetchData();
      }
    },
    resetForm() {
